...
function record_scalar_loop!(histogram, values)
    for value in values
        HH.record_value!(histogram, value)
    end
    return histogram
end

hist = HH.Histogram(1, 1000, 2)
report_benchmark("Histogram record scalar",
    @benchmark(record_scalar_loop!($hist, $values), evals=1), operations=length(values))
...
report_benchmark("Atomic record scalar",
    @benchmark(record_scalar_loop!($atomic, $values), evals=1), operations=length(values))
...
report_benchmark("Concurrent record scalar",
    @benchmark(record_scalar_loop!($concurrent, $values), evals=1), operations=length(values))
...
report_benchmark("Auto concurrent record scalar",
    @benchmark(record_scalar_loop!($auto_concurrent, $values), evals=1), operations=length(values))
...
recorder = HH.Recorder(1, 1000, 2)
report_benchmark("Recorder record scalar",
    @benchmark(record_scalar_loop!($recorder, $values), evals=1), operations=length(values))
...
report_benchmark("SingleWriter record scalar",
    @benchmark(record_scalar_loop!($single, $values), evals=1), operations=length(values))
